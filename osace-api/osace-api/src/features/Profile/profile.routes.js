...
router.get('/:id/badges', verifyToken, async (req, res) => {
  ...
  // GET /my-data — GDPR full data export
  router.get('/my-data', verifyToken, async (req, res) => {
    const userId = req.user.userId;
    try {
      const [userResult, eventsResult, contribResult, badgesResult] = await Promise.all([
        pool.query(
        ...
        pool.query(
          `SELECT e.title, e.start_time, e.end_time, e.category, e.location,
                  ea.confirmation_status, ea.awarded_hours, ea.created_at AS registered_at
        ...
        pool.query(
          `SELECT b.name, b.description, b.icon_name, ub.earned_at
           FROM user_badges ub
